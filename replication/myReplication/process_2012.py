...
filt_data_2012_IN = DATA_DIR+'new_filt_gt1_yt.csv.bz2'
filt_data_2012_OUT = DATA_DIR+'valid_id_gt1_yt.csv.bz2'
EDGEL_OUT = DATA_DIR+'subr_yt_edgelist.txt'
'''
filt_data_2012 = pd.read_csv("data/processed/2012/filt_data_2012.csv")
...
# Retrieve video ids
filt_data_2012['video_id'] = filt_data_2012.url.apply(strip_video_id_from_url)
filt_data_2012 = filt_data_2012[(filt_data_2012.video_id != 'INVALID') & (filt_data_2012.video_id.apply(len) == 11)].copy()

# Create a subreddit-video network
G = nx.Graph()
for i, r in filt_data_2012.iterrows():
    subr = r['subreddit']
    video_id = r['video_id']
...
with open("data/processed/2012/subr_yt_edgelist.txt", 'wb') as f:
    nx.write_weighted_edgelist(G, f)

# Project the subreddit-video network to just the subreddits and output it
subr_proj = bipartite.projection.weighted_projected_graph(G, nodes=filt_data_2012.subreddit.unique())
nx.write_weighted_edgelist(subr_proj, "data/processed/2012/subr_only_el.txt")
...
nx.write_gexf(subr_proj, "data/processed/2012/subrs_only.gexf")
...
ORIGINAL_G = ig.Graph.Read_Ncol("data/processed/2012/subr_only_el.txt", directed=False, weights=True)
JUST_CREATED = ig.Graph.Read_Ncol("data/processed/2012/subr_only_el.txt", directed=False, weights=True)
ORIGINAL_G.isomorphic(JUST_CREATED)
del ORIGINAL_G
del JUST_CREATED

# Read in the subreddit edgelist
g = ig.Graph.Read_Ncol("data/processed/2012/subr_only_el.txt", directed=False, weights=True)
...
with open("data/processed/2012/comm_subrs_10_cpm.txt", 'w') as f:
    for s in flattened:
        f.write("%s\n" % s)


# Read in the subreddits obtained via community detection
with open("data/processed/2012/comm_subrs_10_cpm.txt", 'r') as f:
    comm_subrs = set(f.read().splitlines())

filt_df = filt_data_2012[filt_data_2012.subreddit.isin(comm_subrs)].copy()
...
counts_xtab.to_csv("data/processed/2012/counts_subr_vid_crosst.csv")

xtab = pd.crosstab(df_score['video_id'], df_score['subreddit'], values=df_score['score'], aggfunc=np.sum)
xtab.fillna(0, inplace=True)
logged_xtab = np.log(xtab+1)
logged_xtab.shape
logged_xtab.to_csv("data/processed/2012/score_subr_vid_crosst.csv")


vid_data = pd.read_csv("data/anonymized_video_pid.csv")
vid_data = pd.merge(vid_data, filt_data_2012).drop_duplicates("video_id")
vid_data = vid_data[['video_id', 'bert_score', 'collapsed_pid7']]
vid_data.to_csv("data/processed/2012/video.csv", index=False)